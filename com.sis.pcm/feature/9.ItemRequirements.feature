#Author: Arthi
#Keywords Summary : Journals and Item Requirements
@tag
Feature: JournalCreation,Item Requirements,Invoicing,RevenueRecognition,RetentionRelease,CancelInvoiceAndRetainage

  @tag1
  Scenario: JournalCreation,Item Requirements, Invoicing, RevenueRecognition, RetentionRelease, Cancel Invoice and Retainage
    Given User Login Finance and Operations Application
    Then Wait "500" Seconds
    ...
    And User search the ProjectID for "Feature 7"
    ...
    And User search the ProjectID for "Feature 7"
    And User clicks Apply button
    And User clicks on "Plan" in page header
    And User clicks on "Item requirements" section
    And User clicks on New
    And User enters Item Requirement data
    And User clicks on Save
    And Wait "5" Seconds
    And User clicks on "Manage" in page header
    And User clicks on "Functions" in Manage Section
    And User clicks on "Create purchase order" in Functions
    And User clicks on Include
    And User enters Vendor Account for Item Requirements
    And User enters Purchase Unit Price
    And User clicks on OK
    And Wait "10" Seconds
    And User clicks on "Other" tab in Item Requirement
    Then User gets Purchase Order Number and Reference Type
    And User clicks on "Project" tab in Item Requirement
    And User gets Tranasction ID
    And User clicks on "General" tab in Item Requirement
    And User gets Product Name and Purchase Order Status of Item Requirement
    And User clicks on Close
    And User clicks on "Manage" in page header
    And User clicks on "Item tasks" in "Related information" section
    And Wait "3" Seconds
    And User clicks on "Purchase order" field
    And User search the "Purchase Order ID"
    And User clicks Apply button
    Then User Validates Purchase Order, Vendor Account, Invoice Account, Vendor Name, Purchase Type, Approval Status, Purchase Order Status as "Approved" in Purchase Orders Table
    And User clicks on Purchase Order ID
    And Wait "3" Seconds
    Then User Validate SubContract Purchase Order Status "Open order"
    Then User Validate Approval Status "Approved"
    Then User Validates PurchaseOrderLines in Purchase order
    And User clicks on LineDetails Section
    And User clicks on Project tab in LineDetails Section
    ...
    And User clicks on Purchase tab
    And User clicks "Confirm" in Action Section
    And Wait "10" Seconds
    ...
    And User clicks on "Transaction ID" field
    And User search the "Transaction ID"
    And User clicks Apply button
    Then User Validate Committed Cost Table "Item requirement"
    And User clicks on Close
    ...
    #Then User updates Percent complete in contract and validates Work in place, Amount remaining and Invoice amount from sheet "UpdateContracts1"
    ...
    #And User search the ProjectID for "Feature 2"
    #And User clicks Apply button
    #And User selects the Project ID
    #And Wait "60" Seconds
